...
Una vez modelada la mezcla base se le añadirán diversos productos saborizantes\footnote{Los agentes saborizantes que se podrían utilizar son: fruta, saborizantes naturales o artificiales o una mezcla de estos;
...
El problema planteado originalmente por \cite{Ponce2007} consiste en modelar un gel de pectina compuesto sólo por tres clases de componentes (\textit{M}-componentes) que a su vez son submezclas de componentes del mismo tipo (\textit{m}-componentes), los cuales son:
\begin{enumerate}
...
$$I=\sqrt{\sum\limits_{i=1}^cC_i^2+\sum\limits_{j=1}^aA_j^2}
\mbox{; }
i=1,2,3,\ldots,c
\mbox{; }
j=1,2,3,\ldots,a$$
donde $c$ es el número de cationes, $a$ es el número de aniones, $C_i$ y $A_j$ representan la concentración de cada uno de los cationes y aniones, respectivamente, presentes en la mezcla.
...
Para fines prácticos, en este estudio sólo es de interés el búfer compuesto de ácido cítrico/citrato de sodio en una concentración fija y se ajustará el pH en máximo tres nivele:
...
En el Cuadro~\ref{tab:proceso} se muestran los niveles mínimo y máximo para estas variables.
\begin{table}[!h]
\begin{center}
